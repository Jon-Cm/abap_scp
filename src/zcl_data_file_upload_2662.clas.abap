CLASS zcl_data_file_upload_2662 DEFINITION
 PUBLIC
 FINAL
 CREATE PUBLIC .
  PUBLIC SECTION.
  ...
  PRIVATE SECTION.
    ...
    METHODS: get_html RETURNING VALUE(ui_html) TYPE
                                  string.
ENDCLASS.

CLASS zcl_data_file_upload_2662 IMPLEMENTATION.
...
* extract the filename and fileext from this metadata as well as the raw file data.
        SPLIT request->get_text( ) AT
       cl_abap_char_utilities=>cr_lf INTO TABLE DATA(content).
        READ TABLE content REFERENCE INTO
       DATA(content_item) INDEX 2.
        IF sy-subrc = 0.

          SPLIT content_item->* AT ';' INTO TABLE
         DATA(content_dis).
          READ TABLE content_dis REFERENCE INTO
         DATA(content_dis_item) INDEX 3.
          IF sy-subrc = 0.
            SPLIT content_dis_item->* AT '=' INTO
           DATA(fn) filename.
            REPLACE ALL OCCURRENCES OF `"` IN filename
           WITH space.
            CONDENSE filename NO-GAPS.
            SPLIT filename AT '.' INTO filename
           fileext.
          ENDIF.
        ENDIF.
        DELETE content FROM 1 TO 4. " Get rid of the first 4 lines
        DELETE content FROM ( lines( content ) - 8 ) TO
       lines( content ). " get rid of the last 9 lines
